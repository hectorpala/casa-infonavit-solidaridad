...
# Properties to update (8 more + 2 already done = 10 total)
properties = [
    "casa-venta-portalegre-045360",
    "casa-venta-santa-fe-021450",
    "casa-venta-villa-bonita-pODipRm",
    "estrene-casa-en-villa-fontana-en-esquina",
    "hermosa-casa-en-venta-en-belcantto-residencial-lista-para-habitar",
    "infonavit-solidaridad",
    "residencia-en-venta-en-barrio-san-francisco-la-primavera-01",
    "venta-casa-guadalupe"
]
...
def extract_property_data(html_content):
    ...
    price_match = re.search(r'<title>.*?\$([0-9,]+)', html_content, re.IGNORECASE)
    if not price_match:
...
def update_property(prop_dir):
    ...
    # Pattern for iframe-based maps
    iframe_pattern = r'<!-- Location Map Section[^>]*-->.*?</section>'

    if re.search(iframe_pattern, content, re.DOTALL):
        content = re.sub(iframe_pattern, new_map, content, flags=re.DOTALL)
        with open(index_path, 'w', encoding='utf-8') as f:
            f.write(content)
        print(f"✅ Updated {prop_dir}: {data['price_short']} - {data['title']}")
        return data
    else:
...
# Output summary
print("\n" + "="*80)
print("SUMMARY - LOTE 5:")
print("="*80)
for r in results:
    print(f"✅ {r['dir']}")
    print(f"   {r['price_short']} | {r['title']}")

print(f"\nTotal updated: {len(results)} properties")
print(f"\nIncluding previously updated:")
print(f"✅ casa-venta-casa-en-venta-privada-punta-azul-pBcIydL: $1.98M")
print(f"✅ casa-venta-casa-remodelada-en-infonavit-solidaridad-INFONAVIT-SOLIDARIDAD: $1.65M")
print(f"\n Grand Total: {len(results) + 2} properties")
